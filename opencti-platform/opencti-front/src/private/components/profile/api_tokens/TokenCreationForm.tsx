...
const tokenCreationMutation = graphql`
  mutation TokenCreationFormMutation($input: UserTokenAddInput!) {
...
const TokenCreationForm: FunctionComponent<TokenCreationFormProps> = ({
  ...
  const onSubmit = (values: typeof initialValues, { setSubmitting }: FormikHelpers<typeof initialValues>) => {
    ...
    commitMutation({
      mutation: tokenCreationMutation,
      variables: {
        input: {
          name: values.name,
      ...
      onCompleted: (response: TokenCreationFormMutation$data, errors: readonly PayloadError[] | null | undefined) => {
        ...
        const token = response.userTokenAdd.plaintext_token;
